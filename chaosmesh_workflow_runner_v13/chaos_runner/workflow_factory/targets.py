...
from chaos_runner.discover.rc import (
    fetch_rc_cluster,
    find_rc_leader,
    find_etcd_leader,
    find_rc_followers,
    ...
    for t in case.get("targets", []):
        ...
        if finder=="upc_talker":
            ...
            if ctx.get("rc_cluster_state") is None:
                data, url = fetch_rc_cluster()
                ctx["rc_cluster_state"] = data
                ctx["rc_cluster_url"] = url
            resolved[tid] = find_rc_leader(ctx["rc_cluster_state"])
        elif finder == "rc_followers":
            if ctx.get("rc_cluster_state") is None:
                data, url = fetch_rc_cluster()
                ctx["rc_cluster_state"] = data
                ctx["rc_cluster_url"] = url
            resolved[tid] = find_rc_followers(ctx["rc_cluster_state"])
        elif finder == "rc_pods":
            if ctx.get("rc_cluster_state") is None:
                data, url = fetch_rc_cluster()
                ctx["rc_cluster_state"] = data
                ctx["rc_cluster_url"] = url
            resolved[tid] = find_rc_pods(ctx["rc_cluster_state"])
            ...
            if ctx.get("rc_cluster_state") is None:
                ...
                ctx["rc_cluster_state"] = data
                ctx["rc_cluster_url"] = url
            resolved[tid] = find_etcd_leader(ctx["rc_cluster_state"])
        elif finder == "etcd_followers":
            if ctx.get("rc_cluster_state") is None:
                data, url = fetch_rc_cluster()
                ctx["rc_cluster_state"] = data
                ctx["rc_cluster_url"] = url
            resolved[tid] = find_etcd_followers(ctx["rc_cluster_state"])
        elif finder == "etcd_pods":
            if ctx.get("rc_cluster_state") is None:
                data, url = fetch_rc_cluster()
                ctx["rc_cluster_state"] = data
                ctx["rc_cluster_url"] = url
            resolved[tid] = find_etcd_pods(ctx["rc_cluster_state"])
